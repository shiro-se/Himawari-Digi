...
serve(async (req) => {
  try {
    ...
    if (message.sender !== 'cs' && message.sender !== 'client') {
       return new Response(JSON.stringify({ message: `Ignored system message` }), { headers: { "Content-Type": "application/json" } });
    }
    ...
    return new Response(
      JSON.stringify({ message: `Sent ${subscriptions.length} notifications` }),
